...
{
    for (int i = 0; i < 6; ++i)
        m_tPose.SetRot(i, 0.f, 0.f, 0.f); // ¿¸√º ∆ƒ√˜ ¡§¡ˆ
}
...
void CMonsterAnim::Pose_Walk()
{
    ...
    {
        // ¥Ÿ∏Æ∏¶ ø∑¿∏∑Œ ª∏∞Ì(Z√‡) X√‡¿∏∑Œ »ÁµÈ±‚
        m_tPose.SetRot(2, fLegSwing, 0.f, D3DXToRadian(45.f));  // æ’ø¿∏•
        m_tPose.SetRot(3, -fLegSwing, 0.f, D3DXToRadian(-45.f));  // æ’øﬁ
        m_tPose.SetRot(4, -fLegSwing, 0.f, D3DXToRadian(45.f));  // µ⁄ø¿∏•
        m_tPose.SetRot(5, fLegSwing, 0.f, D3DXToRadian(-45.f));  // µ⁄øﬁ
    }
    else
    {
...
void CMonsterAnim::Pose_Attack(const _float& fTimeDelta)
{
    ...
    else if (m_eType == EMonsterType::SPIDER)
    {
        ...
        if (m_fStateTime < 0.3f)
        {
            m_tPose.SetRot(0, 0.f, 0.f, 0.f);
            m_tPose.SetRot(1, 0.f, 0.f, 0.f);
            m_tPose.SetRot(2, D3DXToRadian(-80.f), 0.f, D3DXToRadian(30.f));  // æ’ø¿∏• ª∏±‚
            m_tPose.SetRot(3, D3DXToRadian(-80.f), 0.f, D3DXToRadian(-30.f));  // æ’øﬁ ª∏±‚
